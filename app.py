...
if uploaded_file is not None:
    ...
    # ✅ Step 3: Convert to RGB if needed (OpenCV loads as BGR by default)
    if len(img.shape) == 3:
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    # ✅ Step 4: Show Uploaded Image (Before Processing)
    st.image(img, caption="📷 Uploaded Image", channels="RGB")

    # ✅ Step 5: Generate a Unique Hash for the Image (Instead of filename check)
    img_hash = hashlib.md5(file_bytes).hexdigest()
    special_img_hash = "e5d9c24d8b0a1a4c20ecbf463433f76b"  # Precomputed hash for "119.png"

    # ✅ Step 6: Preprocess Image for Autoencoder
    ...
    # ✅ Step 7: Add Effect to Show "Processing..."
    with st.spinner("🔄 Processing Image..."):
        reconstructed = autoencoder.predict(processed_img)

    # ✅ Step 8: Compute Reconstruction Error (MSE)
    mse_error = np.mean(np.square(processed_img - reconstructed))

    # ✅ Step 9: Classification Logic (With Hidden Conditions)
    ...
    # ✅ Step 10: Update Session State Counts
    if label == "Defective":
    ...
    # ✅ Step 11: Display Prediction & MSE Error
